...
export default function UploadPasteMarketingRecipients({ onSuccess }) {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      const { data } = await api.post('/materials/recipients/upload-paste', {
        text: text.trim(),
        source: source.trim() || 'Paste Upload',
      });
      const dup = data.skippedDuplicates ? ` (${data.skippedDuplicates} duplicates skipped)` : '';
      const noEmail = data.skippedNoEmail ? ` (${data.skippedNoEmail} without email skipped)` : '';
      setMessage(`${data.message || 'Success'}${dup}${noEmail}`);
      setText('');
  ...
  return (
    ...
    >
      ...
      <p className="muted-text" style={{ marginBottom: 12 }}>
        Copy rows from Google Sheets / Excel (<strong>Ctrl+C</strong>) and paste below (
        <strong>Ctrl+V</strong>). Same 9-column UPSC layout as Upload Leads — each row needs{' '}
        <strong>full_name</strong> + <strong>email</strong> for daily material mail.
      </p>
      <form onSubmit={handleSubmit}>
        ...
        <div className="form-row" style={{ alignItems: 'center', marginTop: 12, flexWrap: 'wrap', gap: 10 }}>
          <input
            className="app-input"
            placeholder="Source label (e.g. Lost leads list)"
            value={source}
          ...
          <button type="submit" disabled={loading || !text.trim()} className="app-btn app-btn-primary">
            {loading ? 'Importing…' : 'Import pasted recipients'}
          </button>
        </div>
      </form>
