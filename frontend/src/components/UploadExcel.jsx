  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const sheets = data.sheetsProcessed?.length
        ...
        : '';
      setMessage(`${data.message || 'Success'}${dup}${sheets}`);
      setFile(null);
  ...
  return (
    <div className="upload-excel">
      <p className="muted-text" style={{ marginBottom: 12 }}>
        Uploads <strong>all tabs</strong> in the file (WebsiteFormData, Prelims confidence Batch,
        FormResponses, Get in touch, etc.). Each row needs <strong>Name</strong> +{' '}
        <strong>Phone or Email</strong>. Source is set from the sheet name.
      </p>
      <div
      ...
      <form onSubmit={handleSubmit} className="form-row" style={{ alignItems: 'center', marginTop: 16 }}>
        <input ref={inputRef} type="file" accept=".xlsx" style={{ display: 'none' }} onChange={(e) => { setFile(e.target.files?.[0] || null); setError(''); }} />
        <span className="muted-text" style={{ marginRight: 12 }}>{file ? file.name : 'No file chosen'}</span>
        <button type="button" onClick={() => inputRef.current?.click()} className="app-btn app-btn-ghost">Browse</button>
